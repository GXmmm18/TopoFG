...
from ..modules.ms_feature_fusion import SimpleBEVEncoder
from mmdet3d.models.detectors.mvx_two_stage import MVXTwoStageDetector
from ...utils.builder import build_bev_constructor, build_bev_fus_neck
from mmcv.cnn.bricks.transformer import build_transformer_layer_sequence
from PIL import Image
import os
import time
@DETECTORS.register_module()
class TopoFG(MVXTwoStageDetector):
    ...
    def __init__(self,
        ...
        super(TopoFG, self).__init__(**kwargs)

        if bev_constructor is not None:
            self.bev_constructor = build_bev_constructor(bev_constructor)
        
        self.bev_encoder = SimpleBEVEncoder(bev_constructor['embed_dims'], bev_constructor['embed_dims'])
        if bev_neck is not None:
            self.bev_neck = build_bev_fus_neck(bev_neck)
    ...
    def forward_train(self,
        ...
        bev_feats = bev_feats.view(bs, self.bev_constructor.bev_h, self.bev_constructor.bev_w, bev_feats.shape[-1]).permute(0, 3, 1, 2).contiguous()
        bev_feats_ms = self.bev_encoder(bev_feats)
        bev_feats_ms = self.bev_neck(bev_feats_ms)
        targets = []
        for b in range(bs):
            targets.append(dict(gt_lanes=gt_lanes_3d[b], \
                                gt_labels = gt_lane_labels_3d[b], \
                                gt_masks_list=gt_masks_list[b]))
        segm_mask_pred_list, ins_query, dn_metas = self.segm_head(bev_feats_ms, targets, pc_range=self.pts_bbox_head.pc_range)       

        mask_dict = dn_metas.pop('mask_dict')
        dn_pad_size = dn_metas["dn_pad_size"]
        dn_metas['mask_pred'] = segm_mask_pred_list[-1].detach()
        outputs_segm_mask_pred_dn, outputs_segm_mask_pred = [], []
        for lvl in range(len(segm_mask_pred_list)):
            outputs_segm_mask_pred_dn.append(segm_mask_pred_list[lvl][:, :dn_pad_size])
            outputs_segm_mask_pred.append(segm_mask_pred_list[lvl][:, dn_pad_size:])
        outputs_segm_mask_pred_dn = torch.stack(outputs_segm_mask_pred_dn)
        outputs_segm_mask_pred = torch.stack(outputs_segm_mask_pred)

        pts_outs = self.pts_bbox_head(bev_feats_ms, img_metas, te_feats, te_cls_scores, ins_query=ins_query, dn_metas=dn_metas)
        pts_outs.update(all_segm_mask_pred=outputs_segm_mask_pred)

        if self.segm_head.dn_enabled and self.training:
            mask_dict["output_known_lbs_bboxes"] = (
                outputs_segm_mask_pred_dn,
                pts_outs.pop('outputs_coords_dn'),
                pts_outs.pop('all_cls_scores_dn'),
                pts_outs.pop('all_lclc_preds_dn')
            )
            mask_dict['lclc'] = gt_lane_adj
        loss_inputs = [pts_outs, mask_dict, gt_masks_list, gt_lanes_3d, gt_lane_labels_3d, gt_lane_adj, gt_lane_lcte_adj, te_assign_result]     
    ...
    def simple_test_pts(self, x, img_metas, img=None, prev_bev=None, rescale=False):
        ...
        bev_feats = bev_feats.view(bev_feats.shape[0], self.bev_constructor.bev_h, self.bev_constructor.bev_w, bev_feats.shape[-1]).permute(0, 3, 1, 2).contiguous()
        bev_feats_ms = self.bev_encoder(bev_feats)
        bev_feats_ms = self.bev_neck(bev_feats_ms)
        
        segm_mask_pred_list, ins_query, dn_metas = self.segm_head(bev_feats_ms)       
        dn_metas['mask_pred'] = segm_mask_pred_list[-1].detach()
        outs = self.pts_bbox_head(bev_feats_ms, img_metas, te_feats, te_cls_scores, ins_query=ins_query, dn_metas=dn_metas)
