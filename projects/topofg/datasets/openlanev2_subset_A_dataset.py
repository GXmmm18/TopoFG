...
class OpenLaneV2_subset_A_Dataset(Custom3DDataset):
    ...
    #         cat_bev_img = np.concatenate([bev_img_gt, divider, bev_img_pred], axis=1)
    #         output_path = os.path.join(out_dir, f'{info["segment_id"]}/{info["timestamp"]}/bev.jpg')
    #         mmcv.imwrite(cat_bev_img, output_path)
    ...
    def show(self, results, out_dir, score_thr=0.3, show_num=20, **kwargs):
        ...
        for idx, result in enumerate(results):
            ...
            bev_img_gt = show_bev_results(gt_lanes, pred_lanes, only='gt', map_size=[-52, 52, -27, 27], scale=20)
            bev_img_pred = show_bev_results(gt_lanes, pred_lanes, only='pred', map_size=[-52, 52, -27, 27], scale=20)
            divider = np.ones((bev_img_pred.shape[0], 7, 3), dtype=np.uint8) * 128
            concat_img = np.concatenate([bev_img_gt, divider, bev_img_pred], axis=1)
            output_path = os.path.join(out_dir, f'{info["segment_id"]}/{info["timestamp"]}/bev.jpg')
            mmcv.imwrite(concat_img, output_path)
            ...
            output_path = os.path.join(out_dir, f'{info["segment_id"]}/{info["timestamp"]}/conn_gt.jpg')
            mmcv.imwrite(conn_img_gt, output_path)

            output_path = os.path.join(out_dir, f'{info["segment_id"]}/{info["timestamp"]}/conn_pred.jpg')
            mmcv.imwrite(conn_img_pred, output_path)
    ...
    def _filter_by_confidence(annotations, threshold=0.3):
        ...
        annotations['topology_lclc'] = annotations['topology_lclc'][lc_mask][:, lc_mask] > 1.0
