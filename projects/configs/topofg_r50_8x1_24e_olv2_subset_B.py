_base_ = []
custom_imports = dict(imports=['projects.bevformer', 'projects.topofg'])
...
data_root = './data/OpenLane-V2/subset_B/'
...
model = dict(
    ...
    lane_head=dict(
        ...
        transformer=dict(
            type='TopoFGTransformerDecoderOnly',
            embed_dims=_dim_,
            pts_dim=pts_dim,
            decoder=dict(
                type='TopoFGSGNNDecoder',
                pc_range=point_cloud_range,
                num_layers=6,
                return_intermediate=True,
                transformerlayers=dict(
    ...
    train_cfg=dict(
        ...
        lane=dict(
            assigner=dict(
                ...
                pc_range=point_cloud_range),  
            segm_assigner=dict(
                type="Mask_HungarianAssigner",
                mask_cost=dict(type="CrossEntropylossCost", weight=5.0, use_sigmoid=True),
                dice_cost=dict(type="DiceCost", weight=2.0, pred_act=True, eps=1.0))))
)
...
train_pipeline = [
    ...
    dict(type='GenerateLaneMask', mask_size=[bev_h_, bev_w_], n_pts_per_vec=method_para['n_points']),
    dict(type='CustomFormatBundle3DLane', class_names=class_names),
    dict(type='CustomCollect3D', keys=[
        'img', 'gt_lanes_3d', 'gt_lane_labels_3d', 'gt_masks_list', 'gt_lane_adj',
